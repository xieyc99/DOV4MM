...
def knn_monitor(net, memory_data_loader, test_data_loader, epoch, k=200, t=0.1, hide_progress=True, classes=-1, subset=False, backdoor_loader=None, trigger=None, poison=None):
    ...
    if subset: 
        ...
        feature_labels = torch.tensor(memory_data_loader.dataset.targets, device=feature_bank.device)  # cifar10 imagenet
        # feature_labels = torch.tensor(memory_data_loader.dataset.labels, device=feature_bank.device, dtype=torch.int64)  # svhn stl10
