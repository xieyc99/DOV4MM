...
def set_aug_diff(args):
    ...
    if 'cifar' in args.dataset  or 'svhn' in args.dataset or 'image' in args.dataset or 'stl10' in args.dataset:
        ...
        if not args.disable_normalize:
            if args.method == 'mocov3':
                ...
                #                                     aug.RandomGrayscale(p=0.2),
                #                                     normalize)
                train_transform = nn.Sequential( aug.RandomResizedCrop(size = (args.size, args.size), scale=(0.5, 1.0)),
                                                    aug.RandomHorizontalFlip(),
                                                    RandomApply(aug.ColorJitter(0.4, 0.4, 0.4, 0.1), p=0.8),
                                                    aug.RandomGrayscale(p=0.2),
                                                    normalize)
    ...
    if args.part != '':
        subset1, subset2 = split_dataset(train_dataset, args.num_classes, args.unlabel, args.ratio, args.closest_path)
        print('subset1:', len(subset1))
        print('subset2:', len(subset2))
        if args.part == 'before':
...
class ImageNet(datasets.ImageFolder):
    def __init__(self, root, train=True, transform=None, sub_class_num=1000, selected_classes=None):
        if train:
        ...
        if transform is None:
            self.transform = transforms.Compose([
                        ...
                        # transforms.Normalize(mean=[0.485, 0.456, 0.406],  # 对图像进行标准化
                        # std=[0.229, 0.224, 0.225])
                    ])
        else:
        ...
        self.imgs, self.targets, self.selected_classes = self._make_dataset(self.root_dir)
        self.targets = torch.tensor(self.targets, dtype=torch.long)
        # print(self.targets[5000:5010])
    ...
    def _make_dataset(self, dir):
        ...
        if self.selected_classes != None:
            ...
            selected_classes = random.sample(os.listdir(dir), self.sub_class_num)
        print('subset classes num:', len(selected_classes))
        print('subset classes:', selected_classes[:10])
        # print('--------------', 'n01558993' in selected_classes)
        for target in sorted(selected_classes):
        ...
        return images, targets, selected_classes
    
class ImageNet_mask(datasets.ImageFolder):
    def __init__(self, root, train=True, transform=None, sub_class_num=1000):
        if train:
            self.root_dir = root+'/train'
        else:
            self.root_dir = root+'/val'
        ...
        if transform is None:
            ...
            self.transform = transform
        self.trans_mask = RandomPatchMasking(patch_size=16, min_mask_ratio=0.25, max_mask_ratio=0.75, mask_value=0, random=False)
        self.sub_class_num = sub_class_num
        self.classes, self.class_to_idx = self._find_classes(self.root_dir)
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            img = self.transform(img)
        mask_img, mask_pos = self.trans_mask(img)

        return img, mask_img, mask_pos, target
...
def split_dataset(train_dataset, num_classes, unlabel=True, ratio=0.5, closest_path=None):
    ...
    if closest_path:
        closest_arr = np.load(closest_path)
        closest_list = closest_arr.tolist()
        subset2 = Subset(subset2, closest_list)

        set_dataset2 = set(split_dataset2)
        set_closest = set(closest_list)
        remaining_elements = list(set_dataset2 - set_closest)
        random_elements = random.sample(remaining_elements, 20000)
        random_subset2 = Subset(train_dataset, random_elements)

        subset2 = ConcatDataset([subset2, random_subset2])

    return subset1, subset2

class WebDataset(datasets.ImageFolder):
    def __init__(self, root, train=True, transform=None):
        ...
        self.root = root
        self.imgs, self.targets = self._make_dataset(root)
        self.targets = torch.tensor(self.targets, dtype=torch.long)
        # print(self.targets[5000:5010])
    ...
    def _make_dataset(self, dir):
        ...
        for fname in sorted(os.listdir(dir)):
            if fname.endswith(".JPEG") or fname.endswith(".png") or fname.endswith(".jpg") or fname.endswith(".PNG") or fname.endswith(".JPG"):
...
class DataAugmentation(object):
    def __init__(self, img_size, part_size, img_mean, img_std, global_crops_scale, local_crops_scale, local_crops_number, is_train=True):
        ...
        # self.global_transfo_t = transforms.Resize(img_size, interpolation=3)
        self.global_transfo_t = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(img_size)])
    ...
    def __call__(self, image):
        ...
        if self.is_train:
            ...
            # crop = self.normlize(crop)
            # print('crop:', self.normlize(crop).shape)
            crops.append(self.normlize(crop))
        else:
            crop = self.global_transfo_t(image)
            # print('test_crop:', self.normlize(crop).shape)
            crops.append(self.normlize(crop))
